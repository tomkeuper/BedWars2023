            ...
            for (Player on : players) {
                on.sendMessage(
                        getMsg(p, Messages.COMMAND_JOIN_PLAYER_JOIN_MSG)
                            .replace("%bw_v_prefix%", BedWars.getChatSupport().getPrefix(p))
                            .replace("%bw_v_suffix%", BedWars.getChatSupport().getSuffix(p))
...
//            BoardManager.getInstance().giveSidebar(p, this, false);
    ...
    public void removePlayer(@NotNull Player p, boolean disconnect) {
        ...
        for (Player on : getSpectators()) {
            on.sendMessage(getMsg(p, Messages.COMMAND_LEAVE_MSG).replace("%bw_v_prefix%", BedWars.getChatSupport().getPrefix(p)).replace("%bw_playername%", p.getName()).replace("%bw_player%", p.getDisplayName()));
        }
                        ...
                        for (String s : getList(p, Messages.GAME_END_TOP_PLAYER_CHAT)) {
                            String message = s
                                    .replace("%bw_first_format%", firstPlayer == null ? getMsg(p, Messages.MEANING_NOBODY) : getMsg(firstPlayer, Messages.GAME_END_FIRST_KILLER)
                                            .replace("%bw_v_prefix%", BedWars.getChatSupport().getPrefix(firstPlayer))
                                            .replace("%bw_v_suffix%", BedWars.getChatSupport().getSuffix(firstPlayer))
                                    ...
                                    .replace("%bw_second_format%", secondPlayer == null ? getMsg(p, Messages.MEANING_NOBODY) : getMsg(secondPlayer, Messages.GAME_END_SECOND_KILLER)
                                            .replace("%bw_v_prefix%", BedWars.getChatSupport().getPrefix(secondPlayer))
                                            .replace("%bw_v_suffix%", BedWars.getChatSupport().getSuffix(secondPlayer))
                                    ...
                                    .replace("%bw_third_format%", thirdPlayer == null ? getMsg(p, Messages.MEANING_NOBODY) : getMsg(thirdPlayer, Messages.GAME_END_THIRD_KILLER)
                                            .replace("%bw_v_prefix%", BedWars.getChatSupport().getPrefix(thirdPlayer))
                                            .replace("%bw_v_suffix%", BedWars.getChatSupport().getSuffix(thirdPlayer))
